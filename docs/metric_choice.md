...
> Сводный документ по выбору метрики и весов: тестирование подходов, формирование весов, вердикт по delta.
>
...
## Разрешение конфликтов конфига

Запуск: `python3 src/metric/analysis/02_resolve_conflicts.py`.

### `search_refinement_rate` direction
...
### Финальный 3-групповой состав

После разрешения обоих конфликтов выше — 16 фичей:
...
## Within-group consensus (per group)
...
- **quality.** PC1 expvar 57% — выше порога, но `km_median_time_to_cart_s` orthogonal (loading ≈ 0). PC1 фактически 2-фичевый индекс. OptMin даёт km median 0.46, повышая min_corr с 0.47 до 0.68, но bootstrap CI на km median = [0.02, 0.78] — практически случайный.
...
Equal — **не от лени, а от данных**: на baseline 61 день consensus-методы (PC1, OptMin) не дают надёжного преимущества над равными весами — это безопасный дефолт при текущем объёме данных.

---

## Between-group решение и sanity

Запуск: `python3 src/metric/analysis/04_between_group_and_sanity.py`.

### Domain priors + consensus diagnostic
...
> Группы engagement (10 фичей) и discovery (3 фичи) сильно redundant на уровне групповых score. На будущее — рассмотреть консолидацию: либо объединить в одну группу «activity», либо переработать discovery (другие фичи, не rate-based).

### Sanity-checks

#### Преемственность с прежним np.mean
При within=Equal новая и старая формулы тождественны (`np.mean` ≡ Equal). Corr = 1.000. Проверка преемственности — формальная.

#### Stress-test: +20% к `dead_search_rate` на 5 дней
...
### Hold-out corrs (corr(δ_score_val, Δtarget_val))
...
- На будущее — рассмотреть EWMA/CUSUM поверх δ_score для drift, который сейчас никто не ловит.

---

## Сводное Production-решение
...
Raw-MAD per metric с порогом median ± 3·1.4826·MAD по baseline. Реализация — `src/metric/anomaly.py` (adaptive DOW, см. `docs/validation.md`).
...
3. **Delta-anomaly не production.** Сохранён как research-инструмент. Документация — см. раздел «Delta как anomaly detector» выше.
4. **NLP-фичи вне композита.** `complex_query_rate`, `in_product_base_rate` в текущем составе Health Score не участвуют. С их включением quality стал бы 5-фичевым, что могло бы повысить PC1 expvar.
5. **n=61 день мал** для bootstrap на 10 фичах. На объёме 3+ месяцев оценки уточняются.

### Что в коде

- `configs/weights.yaml` — единый источник production-весов
- `src/metric/analysis/02_resolve_conflicts.py` — разрешение конфликтов конфига
- `src/metric/analysis/03_within_group_weights.py` — within-group consensus
- `src/metric/analysis/04_between_group_and_sanity.py` — between-group решение и sanity
- `src/metric/analysis/05_delta_anomaly_score.py` — delta hold-out и corrs
- `src/metric/analysis/06_synthetic_injection.py` — синтетические инъекции и вердикт
