# Валидация Health Score (Задача 3)

> **TL;DR.** 3 из 4 проверок PASS, одна — FAIL (C1: FP rate raw-MAD).
...
|---|---|---|---|
| C1 | FP rate raw-MAD per feature (LOO) | `07_fp_rate_baseline.py` | FP ≤ 5% на всех 16 фичах при k=3 |
| C2 | Bootstrap CI на самой метрике | `08_health_bootstrap_ci.py` | width 95% CI median(health) ≤ 5 пунктов |
| C3 | Синтетика 16 фичей × 3 сценария | `09_synthetic_full_coverage.py` | ≥ 80% detected, нет слепых фичей |
| C4 | Чувствительность baseline-окна | `10_baseline_window_sensitivity.py` | spread Δhealth между схемами ≤ 3 п. |

Сводный: `11_validation_summary.py` → `out/validation_summary.json`.

---

## C1. FP rate raw-MAD — **FAIL**
...
## C2. Bootstrap CI на Health — **PASS**
...
## C3. Синтетика 16 × 3 — **PASS** (с оговоркой)
...
|---|---|---|---|---|---|---|
| fixed_first_half | 30 | 0.319 | 100.87 | 100.22 | −0.65 | ✓ |
| rolling_14 | 14 | 0.199 | 100.55 | 100.16 | −0.39 | ✓ |
| rolling_30 | 30 | 0.260 | 100.81 | 100.29 | −0.52 | ✓ |
...
## Итог и план изменений

**Production-метрика Health Score** — валидирована, изменений не требуется.

**Production-алертер raw-MAD** — переделан в `src/metric/anomaly.py`:
`MadDetector` с adaptive DOW-deseasonalization + direction-aware алертами.
...
1. **n=30 baseline недостаточно** для устойчивых DOW-средних на growth-метриках
   (`rate_views`, `rate_favs`, `rate_clicks`). Перепрогнать `13_fp_rate_dow_production.py`
   при baseline≥60.
...
3. **`click_per_view`** — слабая чувствительность к step/drift (см. C3).
   Spike детектится, step/drift — нет. Принять как trade-off или добавить EWMA.
