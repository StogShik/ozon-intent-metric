...
Single source of truth — `configs/weights.yaml`. Все формулы повторяют задачу
№2 (`docs/metric_choice.md`). Anomaly detection реализован в `anomaly.py`,
...
class FeatureSpec:
    name: str
    weight: float
    direction: int  # +1 (positive: больше = лучше); −1 (anti: меньше = лучше)
...
# --------------------------------------------------------------------------- #
# Агрегация day_summary → плоский dict                                        #
# --------------------------------------------------------------------------- #
...
def compute_metrics(df: pd.DataFrame) -> dict[str, float]:
    ...
    # Все фичи из day_summary, кроме служебных
    skip = {"date", "category", "n_users", "n_sessions", "is_cart"}
    for col in df.columns:
        if col in skip:
            continue
        metrics[col] = wmean(col)
    # session_conversion_rate — exact binary
...
def metric_consensus(
    ...
    bootstrap-стабильность (см. `docs/metric_choice.md §A1-A3`), поэтому внутри
...
def stratified_health(
    ...
    for cat, w_cat in base_weights.items():
        ...
        if len(td) == 0 or len(bd) == 0:
            continue
        if int(td["n_sessions"].sum()) < min_sessions_per_cat:
            continue
    ...
    if not contribs:
        return float("nan")
    return float(sum(contribs) / sum(weights_used))
...
# --------------------------------------------------------------------------- #
# Delta-score (DIAGNOSTIC, не production)                                     #
# --------------------------------------------------------------------------- #


@dataclass
class DeltaScoreDiagnostic:
    """Дельта-композит как supervised summary δX → Δtarget.

    ⚠ НЕ production. См. `docs/metric_choice.md §B`: на 5 синтетических сценариях
    ...
    def fit(
        ...
        warnings.warn(
            ...
            "(см. docs/metric_choice.md §B). Используйте MadDetector из anomaly.py.",
            UserWarning,
            stacklevel=2,
        )
        ...
        # Daily aggregated baseline → дельты
        daily = (
        ...
        # km_speed_to_cart как target (положительный направленностью)
